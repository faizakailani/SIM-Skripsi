...
if(isset($_GET['NIDN'])){echo "<a href=insertmasterdosenskripsidetail.php?NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-light'><font face=Verdana color='black' size=2><i class='fa fa-plus'></i>&nbsp;Tambah</font></button></a><br><br>";}
...
$firstColumn = 0;
  ...
  if(isset($_GET['NIDN'])){echo "<td class='align-middle'><a class=linklist href=viewmasterdosenskripsidetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-warning' data-toggle='tooltip' data-placement='top' title='Lihat data'><font face=Verdana size=1><i class='fa fa-eye'></i></font></button></a>";}
  if(isset($_GET['NIDN'])){echo "<a class=linklist href=editmasterdosenskripsidetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Edit data'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a>";}
  if(isset($_GET['NIDN'])){echo "<a class=linklist href=deletemasterdosenskripsidetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Hapus data'><font face=Verdana size=1><i class='fa fa-Trash'></i></font></button></a></td>";}
  echo "</tr>";
  }
echo "</table><br>";
echo "</div>";
