...
<div id="page-wrapper">
  ...
  ?>
    ...
    if (isset($_GET['Kode'])) {
      echo "<a href=insertmasterprogram_studimahasiswadetail.php?Kode=" . mysqli_real_escape_string($con, $_GET['Kode']) . "><button type='button' class='btn btn-light'><font face=Verdana color='black' size=2><i class='fa fa-plus'></i>&nbsp;Tambah</font></button></a><br><br>";
    }
    if ((!isset($_POST["cari"])) or ($_POST["cari"] == "")) {
      ...
      if (isset($_GET['Kode'])) {
        $result = mysqli_query($con, "SELECT * FROM mahasiswa where mahasiswa.Program_Studi= '" . mysqli_real_escape_string($con, $_GET['Kode']) . "' LIMIT $posisi,$batas");
      }
...
<th class='align-middle'><font color=white size=2>Aksi</font></th>
        ...
        if (isset($_GET['Kode'])) {
          echo "<td class='align-middle'><a class=linklist href=viewmasterprogram_studimahasiswadetail.php?NIM=" . $row['NIM'] . "&Kode=" . mysqli_real_escape_string($con, $_GET['Kode']) . "><button type='button' class='btn btn-warning' data-toggle='tooltip' data-placement='top' title='Lihat data'><font face=Verdana size=1><i class='fa fa-eye'></i></font></button></a>";
        }
        if (isset($_GET['Kode'])) {
          echo "<a class=linklist href=editmasterprogram_studimahasiswadetail.php?NIM=" . $row['NIM'] . "&Kode=" . mysqli_real_escape_string($con, $_GET['Kode']) . "><button type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Edit data'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a>";
        }
        if (isset($_GET['Kode'])) {
          echo "<a class=linklist href=deletemasterprogram_studimahasiswadetail.php?NIM=" . $row['NIM'] . "&Kode=" . mysqli_real_escape_string($con, $_GET['Kode']) . " onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger' data-toggle='tooltip' data-placement='top' title='Hapus data'><font face=Verdana size=1><i class='fa fa-Trash'></i></font></button></a></td>";
        }
        echo "</tr>";
      }
      echo "</table><br>";
      ...
      if (isset($_GET['Kode'])) {
        $tampil2 = mysqli_query($con, "SELECT * FROM mahasiswa where mahasiswa.Program_Studi='" . mysqli_real_escape_string($con, $_GET['Kode']) . "'");
      }
